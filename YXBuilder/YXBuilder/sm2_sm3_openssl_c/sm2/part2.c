...
//    DEFINE_SHOW_STRING(Z_A.hash, HASH_BYTE_LENGTH);
    ...
    memset(&e, 0, sizeof(e));
    BUFFER_APPEND_STRING(e.buffer, e.position, HASH_BYTE_LENGTH, Z_A.hash);
    BUFFER_APPEND_STRING(e.buffer, e.position, strlen((const char*)sign->message), (BYTE *)sign->message);
    SM3_Init();
    SM3_Update(e.buffer, e.position);
    SM3_Final_byte(e.hash);
//    DEFINE_SHOW_STRING(e.hash, HASH_BYTE_LENGTH);
//    DEFINE_SHOW_STRING(sign->k, ecp->point_byte_length);
    ...
    BN_sub(tmp1, k, tmp1);
    BN_mod_mul(s, s, tmp1, ecp->n, ecp->ctx);
    int rsLength=ecp->point_byte_length;
    sm2_bn2bin(r, sign->r, rsLength);
    sm2_bn2bin(s, sign->s, rsLength);
...
//    DEFINE_SHOW_BIGNUM(s);
    ...
    int pos1 = 0;
    memcpy(&result[pos1], sign->r+rsLength-32, 32);
    pos1 = pos1 + 32;
    memcpy(&result[pos1], sign->s+rsLength-32, 32);
