...
int sm2_encrypt(ec_param *ecp, message_st *message_data)
{
    ...
    int i;
    sm2_hash local_C_3;
    int c1_length,c2_length,c3_length;
    P_x = BN_new();
    P_y = BN_new();
    k = BN_new();
    ...
    pos1 = 0;
    c1_length= 1 + ecp->point_byte_length + ecp->point_byte_length;
    BUFFER_APPEND_STRING(message_data->C, pos1,c1_length
                         , message_data->C_1);
    
    //	LOGE("SM2_c1_length--->%d", c1_length);
    c2_length=message_data->message_byte_length;
    BUFFER_APPEND_STRING(message_data->C, pos1, c2_length
                         , message_data->C_2);
    //	LOGE("SM2_c2_length--->%d", c2_length);
    c3_length=HASH_BYTE_LENGTH;
    BUFFER_APPEND_STRING(message_data->C, pos1, c3_length
                         , message_data->C_3);
    //	LOGE("SM2_c3_length--->%d", c3_length);
    //	printf("encrypt: \n");
